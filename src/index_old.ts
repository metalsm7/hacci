...
class Hacci {
    ...
    private init2(): void {
        ...
        //
        this.el.querySelectorAll('*').forEach(function (obj: Node, idx: number) {
            //
    ...
    // // 원래 model값에 __prop 속성이 추가됨... 사용X
    // private redefineModel2(prop: string, parent: any = null, model: any = undefined): void {
    //     //
    //     !parent && (parent = this);
    //     typeof(model) === 'undefined' && (model = this._traces.model);
    //     //
    //     const model_prop: string = `__${prop}`;
    ...
    //             //
    //             this.redefineModel2(key, val, model[model_prop]);
    //         }
    //     }
    // }
    ...
    private redefineModel(prop: string, parent: any = null, prev_model_prop: string = null): void {
        //
        if (!this._traces.model.listen) {
            this._traces.model.listen = function(model: any) {

            };
        }
        //
        !parent && (parent = this);
        const model = this._traces.model.value;
        //
        console.log(`redefineModel[${this._id}] - ${JSON.stringify({prop, prev_model_prop})}`);
        //
